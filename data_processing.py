...
# finding player
table3 = Table("Player", players)
my_DB.insert(table3)  # using previous table
selected_table = my_DB.search("Player")
x = selected_table.filter(lambda x : "ia" in x["team"]).filter(lambda x: int(x["passes"]) > 100)
final_player = x.filter(lambda x: int(x["minutes"]) < 200)
...
third_class = selected_table.filter(lambda x: x["class"] == "3")
print("The average cost for third class tickets.")
print(third_class.aggregate(lambda x: sum(x)/len(x), 'fare'))
first_class = selected_table.filter(lambda x: x["class"] == "1")
